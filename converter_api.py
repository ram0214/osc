    ...
    try:
        if request.method == 'POST':  
            ...
            if f :
                ...
                return jsonify({"message":"kindly provide input file"})
        else:
            return jsonify({"message":"kindly send API request with 'POST' methods"}) 
    except Exception as e:
        return jsonify({"message": f"server error {e}"})
...
def pdf_ppt():
    if request.method == 'POST':
        ...
        prs.save("static/files/pdfppt/"+secure_filename(f.filename[:-4]) + ".pptx")
        file_url = f"{request.url_root}static/files/pdfppt/"+secure_filename(f.filename[:-4]) + ".pptx" 
        return jsonify({'file_url': file_url})
...
def qr_generator():
    if request.method == 'POST':  
        ...
        output_file = "static/files/qrcode/qr_code.png"
        generate_img.save(output_file)
    file_url = f"{request.url_root}static/files/qrcode/qr_code.png"
    return jsonify({'file_url': file_url})

@app.route('/bar_generator/', methods = ['GET','POST'])  
@cross_origin()
def bar_generator():
    if request.method == 'POST':  
        number = request.form['text']
        my_code = EAN13(number, writer=ImageWriter())
        output_file = "static/files/barcode/bar_code.png"
        my_code.save(output_file)
    file_url = f"{request.url_root}static/files/barcode/bar_code.png"
    return jsonify({'file_url': file_url})
...
def excel_pdf():
    if request.method == 'POST':  
        ...
        config = pdfkit.configuration(wkhtmltopdf = "static/wkhtmltox/bin/wkhtmltopdf.exe")
        pdfkit.from_file("static/files/excel_pdf/" +secure_filename(f.filename[:-4])+".html", "static/files/excel_pdf/" +secure_filename(f.filename[:-4])+".pdf",configuration=config)
    file_url = f"{request.url_root}static/files/excel_pdf/" +secure_filename(f.filename[:-4])+".pdf"
    return jsonify({'file_url': file_url})
...
def gif_mp4():
    if request.method == 'POST':  
        ...
        f.save(os.path.join(os.path.abspath(os.path.dirname(__file__)),app.config['UPLOAD_FOLDER'],secure_filename(f.filename)))
        input_file = os.path.join(os.path.abspath(os.path.dirname(__file__)),app.config['UPLOAD_FOLDER'],secure_filename(f.filename))
        output_file = "static/files/gifmp4/" + secure_filename(f.filename) [:-4]+ ".mp4"        
        video = mp.VideoFileClip(input_file)
        result = video.write_videofile(output_file)

    file_url = f"{request.url_root}static/files/gifmp4/" +secure_filename(f.filename[:-4])+".mp4"
    return jsonify({'file_url': file_url})
...
def mp4_gif():
    if request.method == 'POST':  
        ...
        f.save(os.path.join(os.path.abspath(os.path.dirname(__file__)),app.config['UPLOAD_FOLDER'],secure_filename(f.filename)))
        input_file = os.path.join(os.path.abspath(os.path.dirname(__file__)),app.config['UPLOAD_FOLDER'],secure_filename(f.filename))
        output_file = "static/files/mp4gif/" + secure_filename(f.filename) [:-4]+ ".gif"   
        video = VideoFileClip(input_file)     
        gif_clip = video.write_gif(output_file)

    file_url = f"{request.url_root}static/files/mp4gif/" +secure_filename(f.filename[:-4])+".gif"
    return jsonify({'file_url': file_url})
...
def gif_webp():
    if request.method == 'POST':  
        ...
        f.save(os.path.join(os.path.abspath(os.path.dirname(__file__)),app.config['UPLOAD_FOLDER'],secure_filename(f.filename)))
        input_file = os.path.join(os.path.abspath(os.path.dirname(__file__)),app.config['UPLOAD_FOLDER'],secure_filename(f.filename))
        output_file = "static/files/gifwebp/" + secure_filename(f.filename) [:-4]+ ".webp"   
        sequence = []
        im = Image.open(input_file)
        for frame in ImageSequence.Iterator(im):
            sequence.append(frame.copy())
        sequence[0].save(output_file, save_all=True,  append_images = sequence[1:])
    file_url = f"{request.url_root}static/files/gifwebp/" +secure_filename(f.filename[:-4])+".webp"
    return jsonify({'file_url': file_url})
